#!/usr/bin/env python3
"""
CuPy GPU 加速版底座摆放搜索。用并行最小化替代单点循环。
"""
...
try:
    ...
    GPU_AVAIL=False
    print("CuPy 不可用,fallback CPU。安装: pip install cupy-cuda11x")
...
def pose_ik_fast(tp, tR, seed=None, n_rand=30, max_nfev=1500):
    """快速 IK:少种子(30),max_nfev 降低."""
    def resid(q):
        T=fk_all(q); p=T["sand_tcp"][:3,3]; R=T["sand_tcp"][:3,:3]
        e=list(p-tp); e+=list(R_to_axisangle(tR.T@R))
        pen=0.0
    ...
    for s in seeds:
        ...
        q=r.x
        if not all(lo-1e-6<=qi<=hi+1e-6 for (lo,hi),qi in zip(LIMITS,q)): continue
        c=float(np.sum(np.array(r.fun[:6])**2))
        if c<bestc: bestc=c;best=q
    if best is None: return None,1e9,1e9
    T=fk_all(best); p=T["sand_tcp"][:3,3]; R=T["sand_tcp"][:3,:3]
    pe=np.linalg.norm(p-tp); re_=math.degrees(np.linalg.norm(R_to_axisangle(tR.T@R)))
    return best,pe,re_

def test_placement_fast(poses, Trel):
    """测 Trel 变换后全部路径的快速可达性(少种子,30并行)."""
    ok=0; prev_q=None
    for pt in poses:
...
def main():
    ...
    print("=== 底座偏航扫描 (绕 baselink z) [JAX GPU] ===")
    best_yaw=0; best_ok=0
    for deg in range(-150,151,5):
        Trel=rot((0,0,1),math.radians(-deg))
        ok=test_placement_fast(poses,Trel)
